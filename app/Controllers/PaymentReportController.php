...
class PaymentReportController extends BaseController
{
     ...
     */
    private function fetchRows(?string $statusFilter): array
    {
        $db = db_connect();
        $rows = $db->table('proforma_invoices pi')
            ...
            ->groupBy(['pi.id', 'pi.proforma_number', 'pi.proforma_date', 'pi.billing_to', 'pi.total_amount', 'pi.total_gst', 'pi.net_amount', 'c.contact_person', 'c.name'])
            ->orderBy('pi.id', 'DESC')
            ->get()
            ->getResultArray();
    ...
    public function list()
    {
        ...
        $rows = $this->fetchRows($status !== '' ? $status : null);
