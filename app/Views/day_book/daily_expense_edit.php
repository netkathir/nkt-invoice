...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
    <h5 class="mb-0">Edit Daily Expense Entry</h5>
...
<div class="card">
    <div class="card-body">
        <form method="post" action="<?= base_url('day-book/daily-expense-form/update/' . $id) ?>" enctype="multipart/form-data" class="row g-3">
            <?= csrf_field() ?>

            <div class="col-12 col-lg-3">
                <label class="form-label">Expense ID</label>
                <input type="text" class="form-control" name="expense_code" value="<?= esc(old('expense_code') ?: (string) ($expense['expense_code'] ?? '')) ?>" placeholder="Auto Generated if left empty">
            </div>
            ...
            <div class="col-12 col-lg-3">
                ...
                <div class="input-group bms-date-wrap">
                    ...
                    <input
                        ...
                        name="expense_date"
                        value="<?= esc(old('expense_date') ?: (string) (($expense['expense_date'] ?? '') ?: date('Y-m-d'))) ?>"
                        tabindex="-1"
                        aria-hidden="true"
                        required
                    >
            ...
            <div class="col-12 col-lg-3">
                ...
                <input type="text" class="form-control" name="description" value="<?= esc(old('description') ?: (string) ($expense['description'] ?? '')) ?>" placeholder="e.g., Printer toner, Taxi fare">
            </div>

            <div class="col-12 col-lg-3">
                <label class="form-label">Amount <span class="text-danger">*</span></label>
                <input type="number" min="0" step="0.01" class="form-control" name="amount" value="<?= esc(old('amount') ?: (string) ($expense['amount'] ?? '')) ?>" required>
            </div>

            <div class="col-12 col-lg-3">
                <label class="form-label">Payment Method <span class="text-danger">*</span></label>
                <?php $pm = (string) (old('payment_method') ?: ($expense['payment_method'] ?? 'Cash')); ?>
            ...
            <div class="col-12 col-lg-3">
                ...
                <input type="text" class="form-control" name="paid_to" value="<?= esc(old('paid_to') ?: (string) ($expense['paid_to'] ?? '')) ?>" placeholder="e.g., John Doe, XYZ Suppliers" required>
            </div>
            ...
            <div class="col-12 col-lg-3">
                ...
                <?php if (! empty($expense['receipt_path'])): ?>
                    <div class="form-text">
                        Current:
                        <?php $fn = basename((string) $expense['receipt_path']); ?>
                        <a href="<?= base_url('day-book/daily-expense-form/receipt/' . $id) ?>" target="_blank" rel="noopener"><?= esc($fn) ?></a>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="col-12">
                ...
                <textarea class="form-control" name="remarks" rows="3" placeholder="Additional remarks or comments (optional)"><?= esc(old('remarks') ?: (string) ($expense['remarks'] ?? '')) ?></textarea>
            </div>

            <div class="col-12 d-flex justify-content-end gap-2 mt-2">
                <a class="btn btn-secondary" href="<?= base_url('day-book/daily-expense-form') ?>">Cancel</a>
                <button class="btn btn-primary" type="submit">Update Daily Expense Entry</button>
            </div>
        </form>
    </div>
</div>
